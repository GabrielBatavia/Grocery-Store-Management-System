...
def plot_attention_heatmap(attention_scores, input_tokens, output_tokens, example_idx, cmap='viridis', annot=False, color_bar=True, vmin=0, vmax=1):
    ...
    vmin : float, optional
        Minimum value for the heatmap scale, by default 0.
    vmax : float, optional
        Maximum value for the heatmap scale, by default 1.
...
def visualize_random_samples_attention(model, tokenizer, eval_data, num_samples=3, use_fixed_length_padding=False):
    ...
    -------
    list
        A list of filenames where the heatmaps were saved.
    """
        ...
        for cmap in cmaps:
            for annot in annot_options:
                for vmin, vmax in vmin_vmax_options:
                    ...
                    plot_attention_heatmap(attention_scores, input_tokens, output_tokens, i, cmap=cmap, annot=annot,
                                           color_bar=True, vmin=vmin, vmax=vmax)
